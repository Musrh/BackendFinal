...
export const exportStore = async (ownerUid) => {
  ...
  const [slugSnap, ordersSnap, fordersSnap, prodSnap, subSnap] = await Promise.all([
    db.collection("slugs").where("uid", "==", ownerUid).get(),
    db.collection("orders").where("ownerUid", "==", ownerUid).get(),
    db.collection("forders").where("ownerUid", "==", ownerUid).get(),
    db.collection("prodinfos").where("ownerUid", "==", ownerUid).get(),
    db.collection("subscriptions").where("ownerUid", "==", ownerUid).get(),
  ])
  ...
  const exportData = {
    meta: { exportedAt: new Date().toISOString(), ownerUid, siteName: userData.siteName || "", publishedSlug: userData.publishedSlug || "", plan: userData.plan || "free", version: "1.0" },
    store: { user: userData, slugs, orders, forders, prodinfos, subscriptions },
    stats: { totalOrders: orders.length + forders.length, proOrders: orders.length, freeOrders: forders.length,
...
export const exportStoreRoutes = (app) => {
  app.get("/api/admin/export-store/:uid", async (req, res) => {
    ...
    try {
      ...
      if (!ADMIN_EMAILS.includes(decoded.email?.toLowerCase())) return res.status(403).json({ error: "Non autorisé" })
